    ...
    foreach( array_keys( $block_arr ) as $i ) {
        ...
        if ( $block_arr[$i] -> getVar( "visible" ) != 1 ) {
            ...
            if ( $block_arr[$i] -> getVar( "block_type" ) == 'D' || $block_arr[$i] -> getVar( "block_type" ) == 'C' ) {
                $delete_article = "<br /><a href='admin.php?fct=blocksadmin&amp;op=delete&amp;bid=$bid'>" . _DELETE . "</a>" ;
            } else {
                $delete_article = '' ;
            } 
            ...
            if ( $can_clone ) {
                $clone_article = "<br /><a href='admin.php?fct=blocksadmin&amp;op=clone&amp;bid=$bid'>" . _CLONE . "</a>" ;
            } else {
                $clone_article = '' ;
            }
            // displaying part
		...
		<tr valign='middle'>
			...
			<td class='$class' style='text-align: center;' nowrap='nowrap' width='120px'>
			...
			<td class='$class' style='text-align: center;'>
				<a href='admin.php?fct=blocksadmin&amp;op=edit&amp;bid=$bid'>" . _EDIT . "</a>{$delete_article}{$clone_article}
