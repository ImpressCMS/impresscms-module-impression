	...
	if ( $impressionModuleConfig['displayicons'] != 3 ) {
		if ( $newdate < $time ) {
			if ( intval( $status ) > 1 ) {
				if ( $impressionModuleConfig['displayicons'] == 1 )
					$new = '&nbsp;<span class="impression_gradient impression_microbutton"><img src="' . ICMS_URL . '/modules/' . basename( dirname(  dirname( __FILE__ ) ) ) . '/images/icon/icon_alert.gif" alt="" />&nbsp;' . _MB_IMPRESSION_UPDATED . "</span>";
				if ( $impressionModuleConfig['displayicons'] == 2 )
					$new = '<i>' . _MB_IMPRESSION_UPDATED . '</i>';
			} else {
				if ( $impressionModuleConfig['displayicons'] == 1 )
					$new = '&nbsp;<span class="impression_gradient impression_microbutton"><img src="' . ICMS_URL . '/modules/' . basename( dirname(  dirname( __FILE__ ) ) ) . '/images/icon/icon_flag_blue.gif" alt="" />&nbsp;' . _MB_IMPRESSION_NEW . "</span>";
				if ( $impressionModuleConfig['displayicons'] == 2 )
		...
		if ( $popdate > $time ) {
			if ( $counter >= $impressionModuleConfig['popular'] ) {
				if ( $impressionModuleConfig['displayicons'] == 1 )
					$pop = '&nbsp;<span class="impression_gradient impression_microbutton"><img src="' . ICMS_URL . '/modules/' . basename( dirname(  dirname( __FILE__ ) ) ) . '/images/icon/icon_favourites.gif" alt="" />&nbsp;' . _MB_IMPRESSION_POPULAR . "</span>";
				if ( $impressionModuleConfig['displayicons'] == 2 )
